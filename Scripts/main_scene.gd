...
func _build_gameplay_map() -> void:
	var map_data_list: Array[LevelMapData] = SceneManager.current_level_map_data
	if map_data_list.is_empty():
		return
	...
	for map_data in map_data_list:
		if map_data == null or map_data.chunk_hex_coords.is_empty():
			continue
		var map_chunk_centers: Dictionary = HexMapUtils.compute_chunk_centers(
			map_data.chunk_hex_coords, CHUNK_RADIUS, gameplay_tilemap
		)
		for relative_coord in map_data.chunk_hex_coords:
			var center: Vector2i = map_chunk_centers[relative_coord]
...
func reset_for_level():
	...
	LevelManager.start_level(SceneManager.current_level_map_data)
...
func _process(delta: float) -> void:
	...
	if transition != null and transition.material is ShaderMaterial:
		var progress: float = transition.material.get_shader_parameter("progress")
		transition.material.set_shader_parameter("progress", progress + 1.0 *delta)
	cam_time +=  delta
